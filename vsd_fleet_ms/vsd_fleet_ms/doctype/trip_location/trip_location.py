...
class TripLocation(NestedSet):
	...
	def get_location_features(self):
		if not self.location_latitude_longitude:
			return []

		features = json.loads(self.location_latitude_longitude).get("features")
	...
	def set_location_features(self, features):
		...
		location = json.loads(location_latitude_longitude)
	...
	def update_ancestor_location_features(self):
		...
		for ancestor in self.get_ancestors():
			ancestor_doc = frappe.get_doc("Trip Locations", ancestor)
	...
	def add_child_property(self):
		...
		for index, feature in enumerate(filter_features):
			feature["properties"].update({"child_feature": True, "feature_of": self.description})
...
def on_doctype_update():
	frappe.db.add_index("Trip Locations", ["lft", "rgt"])
